import type { CityEdits, DistrictData, EdgeMetrics, RoadEdge } from '../types';
import { multiplierAtHour } from '../types';
...
// congestion is volume/capacity, can exceed 1.0 when oversubscribed — banded
// Smooth 0-40 / Moderate 40-70 / Heavy 70-90 / Severe 90+ as a 0-100 "congestion score".
...
export function computeEdgeMetrics(
  ...
  const effectiveFlow = baseFlowWithTrips * multiplierAtHour(hour);
  ...
  // Queueing: once demand exceeds capacity, the excess backs up physically along the road.
  const overCapacity = Math.max(0, congestion - 1);
  const queueLengthM = overCapacity * edge.lengthM * 0.6;
  const throughputVehPerHr = Math.min(effectiveFlow, capacity);

  return { edgeId: edge.id, effectiveFlow, capacity, congestion, speed, waitingTimeSec, queueLengthM, throughputVehPerHr };
}
...
export function computeAllEdgeMetrics(
  ...
  for (const edge of district.roads) result.set(edge.id, computeEdgeMetrics(edge, edits, baseFlows, hour));
  return result;
}
